...
# ╔═╡ 0f3e2a59-42dc-4a78-a36c-cc54b160adf8
a = Grid(3,3).elements[1].local_node[2]
...
# ╔═╡ b6a6e1c5-0414-4a25-8700-9a1ce98ac9cb
dxdz(GRID, 200)(55,13)

# ╔═╡ b8b23b53-def6-4a07-a474-2f342fec4a40
...
# ╔═╡ 8725543e-46f3-49b5-8e75-6dd483ff7ceb
dxdz2(GRID, index) = (ζ, η)->dϕdζ.*extract_x(GRID, index) |> sum

# ╔═╡ 0cdf5c5a-5f37-4ed6-ad61-d322b9c4eaa8
dxdn2(GRID, index) = (ζ, η)->dϕdη.*extract_x(GRID, index) |> sum

# ╔═╡ e72cdceb-580e-444f-8285-1a2428e3d129
dydz2(GRID, index) = (ζ, η)->dϕdζ.*extract_y(GRID, index) |> sum

# ╔═╡ d0675389-4d9f-46cc-9ef4-528bfc2a5f0a
dydn2(GRID, index) = (ζ, η)->dϕdη.*extract_y(GRID, index) |> sum

# ╔═╡ a7fe2e40-9a87-4edf-9441-6b8c8d53a4b1
dydz2(GRID, 20)(10,10)
...
# ╔═╡ ada95bd4-263a-4f69-b266-ab49d4d81e5d
function calculate_O(grid)
	n_elements = grid.elements |> length
	N_max = grid.shape[1]*grid.shape[2]
	O = OffsetArray(zeros(N_max, N_max), 0:N_max-1, 0:N_max-1)
	
	for m in 0:n_elements-1, i in 1:3, j in 1:3
		p = grid.elements[m].local_node[i]
		q = grid.elements[m].local_node[j]
		f = (ζ, η)->get_Jm(grid,m)(ζ, η)*(ϕ[i](ζ, η)*ϕ[j](ζ, η))
		O[p,q] = O[p,q] + integrateLQTTriangle(f)
	end
	return O
end

# ╔═╡ 1ea6d8e7-ab66-410d-8cfd-c18fcfe3ce04
function WB(grid, E, O)
	n_elements = grid.elements |> length
	for (element_index, element) in enumerate(grid.elements)
		cords = get_triangle_cords(grid, element_index-1)
		for (i, node) in enumerate(cords)
			x, y = node
			if abs(x) ≈ x_max || abs(y) ≈ y_max 
				p = element.local_node[i]
				E[p,:] .= 0
				E[:,p] .= 0
				O[p,:] .= 0
				O[:,p] .= 0
...
# ╔═╡ df6dc61a-f63b-4a6f-b264-964adf7d8f89
GREED = Grid(nx,ny)

# ╔═╡ eb8a20c5-e72d-42d7-b1f9-d513b478980c
GREED

# ╔═╡ e2bffa92-6538-4198-8fa8-69f3b84cffcc
1:10 .|> i->get_Jm(GREED, i)(100,100)

# ╔═╡ 8184379c-17ad-4cba-84d4-e824b2803393
E1 = calculate_E(GREED)

# ╔═╡ f0486392-5a5c-44de-bbe0-4940d302807e
O1 = calculate_O(GREED)

# ╔═╡ 47e5b606-c53f-49d0-a0f7-d45f2e8c01cf
E1b, O1b = WB(GREED, E1, O1)

# ╔═╡ 95d13536-2017-43c0-8049-da75e7d84d6c
E1b

# ╔═╡ ae658cac-06ea-4eb5-ab61-7458c0e40925
O1b

# ╔═╡ c6b571d2-e815-4081-ac0d-e464415b3a8b
λ, vecs = eigen(E1b |> OffsetArrays.no_offset_view, O1b |> OffsetArrays.no_offset_view)

# ╔═╡ a5d31fb6-bad6-420e-b9df-99834842f8c8
vecs[:,1]

# ╔═╡ 429e65fc-8963-478d-9a11-6241e3166ef9
λ[1:10]

# ╔═╡ 00805e9a-e0ff-417f-b7dc-033b8f8da750
GRID

# ╔═╡ 4b5a687a-d1d0-4a63-95c5-a51591398810
l2_norm(p1, p2) = (p1 - p2).^2 |> sum |> sqrt

# ╔═╡ ad9d4a73-7a82-4cfd-931b-aa6f7e0bd55a
function area_of_triangle(p1, p2, p3)
	p1, p2, p3 = (p1,p2,p3) .|> collect 
	a = l2_norm(p1,p2)
	b = l2_norm(p1,p3)
	c = l2_norm(p2,p3)
...
# ╔═╡ 3ba66105-f4a1-4e20-84ce-f1723b30384b
function is_point_inside_element(point, grid, index)
	p1, p2, p3 = get_triangle_cords(grid, index)
	A = area_of_triangle(p1,p2,p3)
	a1 = area_of_triangle(p1,point,p2)
	a2 = area_of_triangle(p2,point,p3)
	a3 = area_of_triangle(p1,point,p3)

	return A ≈ a1 + a2 + a3
end

# ╔═╡ 684d7ad6-e908-4e95-97c1-3705bca98f06
is_point_inside_element((-4,4), GRID, 2)

# ╔═╡ df7dd888-a685-449f-a604-599f2febdd23
function in_which_element(point, grid)
	for (index, element) in enumerate(grid.elements)
		if is_point_inside_element(point, grid, index-1)
			return index-1
		end
	end
	return -1
...
# ╔═╡ 9bfb2f23-5d45-486c-8e6c-1a553818c3f3
function x_y_to_ζ_η(x, y, grid)
	index = in_which_element((x,y), grid)
	p0, p1, p2 = get_triangle_cords(grid, index)
	x0, y0 = p0
	x1, y1 = p1
	x2, y2 = p2
	
	M = -y0*x2 + y0*x1 - y1*x0 + y1*x2 + y2*x0 - y2*x1
	Aζ(x,y) = -y1*x0 - y2*x0 + 2y*x0 - 2y0*x + y1*x2 + y0*x2 - 2x2*y + y0*x1 + 2x*y2 - y2*x1
	Aη(x,y) = y0*x1 + y0*x2 - 2y0*x - y1*x0 - y1*x2 + 2y1*x - y2*x0 + y2*x1 + 2y*x0 - 2y*x1
...
# ╔═╡ 3c1afecb-fa93-416d-b047-01936bfe54ba
x_y_to_ζ_η(1,1, GRID)
...
# ╔═╡ 2614a0cb-2217-4b45-bac7-558f7d4a97db
function get_uk(grid, c)
	N_troj = 3
	fnc(x, y) = begin
		val = 0
		for element in grid.elements, i in 1:3
			l = element.local_node[i]
			ζ, η = x_y_to_ζ_η(x, y, grid)
			val = val + c[l+1]*ϕ[i](ζ, η)
		end
		return val
	end
	return fnc
...
# ╔═╡ e550d7ff-37e4-4947-9b9a-063be489e5f0
u1 = get_uk(GREED, vecs[:, 1])

# ╔═╡ cf05cbd1-a99c-4e62-9577-ab3dce098b51
z = Surface(u1, -5:0.1:5, -5:0.1:5)

# ╔═╡ 902786b8-d060-48c4-85ed-1f2a756e0bb4
heatmap(-5:0.1:5, -5:0.1:5, z)

# ╔═╡ 684681b2-9c5d-4caa-9b5a-0aa25621c19a


# ╔═╡ b810a2c2-52b4-4ba6-9d86-74f1717787b6
md"## Druga część"

# ╔═╡ 2b56b4f7-ed33-45b4-a530-c665fa59edba
y(t, c₁, c₂, ω₁, ω₃) = cos(ω₂*t)*c₂ + sin(ω₃*t)*c₃

# ╔═╡ e36178e3-bc33-446e-8b62-88c1d5299745
y_next(E, O, yk, vk) = (O*yk + Δt*O*vk + (2β-1)/2*Δt^2*E*yk )\(O+β*Δt^2*E)

# ╔═╡ 97cdf839-427a-4e25-a940-d0d7b36cbd77
v_next(E, O, yk_next, yk, vk) = (O*vk - Δt/2*(E*yk + E*yk_next) )\O
